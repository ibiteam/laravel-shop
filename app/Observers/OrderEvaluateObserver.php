...
class OrderEvaluateObserver
{
    ...
    {
        // 审核通过
        if ($order_evaluate->isDirty('status')) {
            if ($order_evaluate->status == OrderEvaluate::STATUS_SUCCESS) {
            ...
            } elseif ($order_evaluate->status == OrderEvaluate::STATUS_REJECT) {
                $order_evaluate_count = OrderEvaluateCount::query()->whereGoodsId($order_evaluate->goods_id)->first();

                if (! $order_evaluate_count instanceof OrderEvaluateCount) {
                    return;
                }

                if ($order_evaluate_count->total >= 1) {
                    $order_evaluate_count->total -= 1;
                }

                if (! empty($order_evaluate->images) && $order_evaluate_count->has_image_total >= 1) {
                    $order_evaluate_count->has_image_total -= 1;
                }
                ...
                foreach ($rank_fields as $field => $counter) {
                    if ($order_evaluate->$field > 4 && $order_evaluate_count->$counter >= 1) {
                        $order_evaluate_count->$counter -= 1;
                    }
                }
