...
it('test payment method index api interface', function () {
    $response = $this->doGet('/api/manage/payment/method', ['name' => '微信', 'is_enabled' => true, 'page' => 1, 'number' => 15]);
    $this->assertArrayHasKey('code', $response);
    $this->assertArrayHasKey('data', $response);
    $this->assertEquals(200, $response['code']);
});
it('test payment method edit api interface', function () {
    $response = $this->doGet('/api/manage/payment/method/edit', ['id' => 1]);
    $this->assertArrayHasKey('code', $response);
    $this->assertArrayHasKey('data', $response);
    $this->assertEquals(200, $response['code']);
});
it('test payment method update api interface', function () {
    ...
    $response = $this->doPost('/api/manage/payment/method/update', $data);
    $this->assertArrayHasKey('code', $response);
    $this->assertArrayHasKey('data', $response);
    $this->assertEquals(200, $response['code']);
});
it('test payment method change field api interface', function () {
    ...
    $response = $this->doPost('/api/manage/payment/method/change/field', $data);
    $this->assertArrayHasKey('code', $response);
    $this->assertArrayHasKey('data', $response);
    $this->assertEquals(200, $response['code']);
});
