<style>
    /*广告位1*/
    .home-advertising-two .carousel-item,
    .home-advertising-two .carousel-item img { width: 100%; height: 100% }
    .home-advertising-two .carousel-item { border-radius: 20px; overflow: hidden; }
    .home-advertising-two .carousel-indicators li { width: 10px; height: 10px; border-radius: 20px; background-color: rgba(255, 255, 255, 0.5); }
    .home-advertising-two .carousel-indicators li.active { width: 26px; background-color: #ffffff; }
</style>

<script type="text/x-template" id="home-advertising-two">
    <div class="temp-item home-advertising-two template-banner two template-public-width" style="min-height: 190px;" :class="{ 's-flex ai-ct jc-ct': temp_item.activity_info && !temp_item.activity_info.length, active: active_index == temp_item.id, disabled: !temp_item.is_show }">
        ...
        <template>
            <div class="s-flex ai-ct flex-wrap" v-if="!temp_item.data || computedDataType(temp_item.data) == 'Array' || !temp_item.data.items.length">
                <div class="template-data-null border-line backg-color-white s-flex ai-ct jc-ct" v-for="item in 2" style="width: 350px; height: 190px;">未上传图片</div>
            </div>
            <ul class="s-flex ai-ct flex-wrap jc-bt" v-else>
                <li class="img-set-full" style="height: 190px;" v-for="item in temp_item.data.items" :style="{ width: temp_item.data.width + 'px', height: temp_item.data.height + 'px' }">
                                ...
                                <el-form-item label="图片宽高" label-width="80px" :prop="'height'" :rules="
                                    [
                                        ...
                                        { validator: (rule, value, callback) => {
                                            if (value < 190 || value > 250) {
                                                callback(new Error('图片高度范围是190px~250px'));
                                            } else if (isNaN(value)) {
                                    ...
                                    <p class="warning-text" style="line-height: 2; margin: 10px 0;">高度范围：190px~250px，只支持设置为整数</p>
                                </el-form-item>
                            </div>
                        </div>
                        <div class="drawer-item">
                            ...
                            <div class="drawer-item-dd">
                                <div class="s-flex ai-ct jc-bt" style="margin: 20px 0;">
                                    ...
                                    <div class="drawer-btn s-flex ai-ct jc-ct cursorp" :class="{ disabled: templateSetForm.data.length >= 20 }" @click="handleClickAddImageData({ target: 'data', max: 20, length: templateSetForm.data.length, item: { 'image': '', 'alias': 'https', 'value': '', 'url': { 'alias': 'https', 'value': '', }, 'sort': 1, 'is_show': 1 } })">
...
<script>
     ...
     * initial_index 指定选中项
     * **/
    Vue.component('home-advertising-two', {
        props: {
            temp_index: {
                type: Number,
                default: 0
        ...
        methods: {
            ...
            async handleClickAddData (dom, len, warning_text, data) {
                ...
                /*const is_null = newValue.filter(item => item.image == '' || (item.alias != '' && item.value == ''))
                if (is_null.length) {
                    this.msg_error && this.msg_error.close()
            ...
            handleClickSubmitSetForm (formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        if (!this.templateSetForm.data.length || this.templateSetForm.data.length < 2) {
                            this.msg_error && this.msg_error.close()
                            this.msg_error = this.$message.error('至少添加2个广告图')
                            return false
                        }
                        if (this.templateSetForm.data.length % 2 != 0) {
                            this.msg_error && this.msg_error.close()
                            this.msg_error = this.$message.error('广告图2个/组，图片个数必须是2的倍数')
                            return false
                        }
