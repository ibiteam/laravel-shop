<style>
    /*广告位1*/
    .home-advertising-three .carousel-item,
    .home-advertising-three .carousel-item img { width: 100%; height: 100% }
    .home-advertising-three .public_text img { width: 32px; height: 32px; line-height: 1; margin-right: 16px; }
    .home-advertising-three .carousel-item { border-radius: 20px; overflow: hidden; }
    .home-advertising-three .carousel-indicators li { width: 10px; height: 10px; border-radius: 20px; background-color: rgba(255, 255, 255, 0.5); }
    .home-advertising-three .carousel-indicators li.active { width: 26px; background-color: #ffffff; }
    /*广告位3*/
    .home-advertising-three * { box-sizing: border-box; }
    .home-advertising-three.three ul { padding: 0; }
    .home-advertising-three.three ul li.img-set-full { margin-bottom: 10px; border-radius: 20px; overflow: hidden; }
    .template-banner-swiper { overflow: hidden; }
    .template-banner-swiper .swiper-slide { height: var(--swiper-height); border-radius: 20px; overflow: hidden; }
</style>

<script type="text/x-template" id="home-advertising-three">
    <div class="temp-item home-advertising-three template-banner three template-public-width" style="min-height: 130px;" :class="{ 's-flex ai-ct jc-ct': temp_item.activity_info && !temp_item.activity_info.length, active: active_index == temp_item.id, disabled: !temp_item.is_show }">
        ...
        <template>
            <div class="s-flex ai-ct flex-wrap jc-bt" v-if="!temp_item.data || computedDataType(temp_item.data) == 'Array' || !temp_item.data.items.length">
                <div class="template-data-null border-line backg-color-white s-flex ai-ct jc-ct" v-for="item in 3" style="width: 230px; height: 280px;">未上传图片</div>
            </div>
            <template v-else>
                <template v-if="temp_item.data.is_show_title == 1">
                    <div class="public-title" style="padding: 0; background-color: transparent;">
                        <div class="public_text s-flex ai-ct">
                            <img v-if="temp_item.data.icon" :src="temp_item.data.icon" />
                            <p>@{{ temp_item.data.title }}</p>
                            {{--<p>热销商品</p>--}}
                        </div>
                ...
                <ul class="s-flex ai-ct flex-wrap" style="margin: 0 -5px;" v-if="temp_item.data.show_style == 1 || (temp_item.data.show_style == 2 && temp_item.data.items.length <= 3)">
                    <li class="img-set-full" v-for="item in temp_item.data.items" :style="{ width: temp_item.data.width + 'px', height: temp_item.data.height + 'px', margin: '0 4px 10px 4px' }">
                        <img :src="item.icon" alt="">
                    </li>
                </ul>
                <div :style="{ '--swiper-height': temp_item.data.height + 'px', 'height': temp_item.data.height + 'px' }" v-if="temp_item.data.show_style == 2 && temp_item.data.items.length > 3">
                    <div class="swiper template-banner-swiper s-flex ai-ct flex-wrap jc-bt" :data-height="temp_item.data.height" :class="`template-banner-swiper${temp_index}`">
                ...
                <div style="height: calc(100vh - 160px); overflow-y: auto;" ref="drawerBox">
                    <div class="drawer-item">
                        ...
                        <div class="drawer-item-dd">
                            <el-form-item label="展示样式" label-width="110px" :prop="'show_style'" :rules="{ required: true, message: '请选择展现样式', trigger: 'change' }">
                                <el-radio-group v-model="templateSetForm.show_style">
                                    <el-radio :label="1">平铺展示</el-radio>
                                    <el-radio :label="2">滚动轮播</el-radio>
                                </el-radio-group>
                                <div class="drawer-upload-warning">
                                    <p>平铺展示：≤3个图片，静态展示。＞3个图片，从左到右从上到下依次展示；</p>
                                    <p>滚动轮播：≤3个图片，静态展示。＞3个图片，循环滚动轮播，单张切换。</p>
                                </div>
                            </el-form-item>
                            <el-form-item label="标题样式" label-width="110px" :prop="'is_show_title'" :rules="{ required: true, message: '请选择展现样式', trigger: 'change' }">
                                <el-radio-group v-model="templateSetForm.is_show_title">
                                    <el-radio :label="1">显示</el-radio>
                                    <el-radio :label="0">隐藏</el-radio>
                                </el-radio-group>
                            </el-form-item>
                            <template v-if="templateSetForm.is_show_title == 1">
                                            ...
                                            <div class="s-flex ai-ct">
                                                <el-tooltip
                                                    ...
                                                    <div slot="content" class="drawer-upload-warning">
                                                        <div class="drawer-preview-icon s-flex ai-ct jc-ct" @click="image_view = 'https://cdn.toodudu.com/uploads/2024/01/10/ad_three_view.png', is_show_imageview = true">
                                                            <em class="iconfont cursorp">&#xe7c3;</em>
                                                        </div>
                                                        <el-image style="width: 100px; height: 100px" src="https://cdn.toodudu.com/uploads/2024/01/10/ad_three_view.png"></el-image>
                                                    </div>
                        ...
                        <div class="drawer-item">
                            ...
                            <div class="drawer-item-dd" style="padding: 0 20px;">
                                ...
                                <el-form-item label="图片宽高" label-width="80px" :prop="'height'" :rules="
                                    [
                                        ...
                                        { validator: (rule, value, callback) => {
                                            if (value < 280 || value > 400) {
                                                callback(new Error('图片高度范围是280px~400px'));
                                            } else if (isNaN(value)) {
                                    ...
                                    <p class="warning-text" style="line-height: 2; margin: 10px 0;">高度范围：280px~400px，只支持设置为整数</p>
                                </el-form-item>
                            </div>
                        </div>
                        <div class="drawer-item">
                            ...
                            <div class="drawer-item-dd">
                                <div class="s-flex ai-ct jc-bt" style="margin: 20px 0;">
                                    ...
                                    <div class="drawer-btn s-flex ai-ct jc-ct cursorp" :class="{ disabled: templateSetForm.data.length >= 15 }" @click="handleClickAddImageData({ target: 'data', max: 15, length: templateSetForm.data.length, item: { 'image': '', 'alias': 'https', 'value': '', 'url': { 'alias': 'https', 'value': '', }, 'sort': 1, 'is_show': 1 } })">
                                        <em class="iconfont">&#xe620;</em>
                                        <label>添加（@{{ templateSetForm.data.length }}/15）</label>
                                    </div>
                                </div>
                                <dl>
                                    <dt>
...
<script>
     ...
     * initial_index 指定选中项
     * **/
    Vue.component('home-advertising-three', {
        props: {
            temp_index: {
                type: Number,
                default: 0
        ...
        template: '#home-advertising-three',
        methods: {
            /** 点击打开组件设置弹窗 */
            ...
            handleClickSubmitSetForm (formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        if (!this.templateSetForm.data.length || this.templateSetForm.data.length < 3) {
                            this.msg_error && this.msg_error.close()
                            this.msg_error = this.$message.error('至少添加3个广告图')
                            return false
                        }
                        if (this.templateSetForm.data.length % 3 != 0) {
                            this.msg_error && this.msg_error.close()
                            this.msg_error = this.$message.error('广告图3个/组，图片个数必须是3的倍数')
                            return false
                        }
                        ...
                        this.doPost('{!! route('manage.app_web_decoration.get_content_data_by_alias') !!}', info).then(res => {
                            if (res.code == 200) {
                                ...
                                setTimeout(() => {
                                    if (res.data.show_style == 2 && res.data.items.length > 3) {
                                        //  如果轮播实例化存在，则销毁重新注册
                                        if (this[`template_banner_swiper${this.temp_index || 0}`]) {
                                            this[`template_banner_swiper${this.temp_index || 0}`].destroy()
                                        }
                                        this[`template_banner_swiper${this.temp_index || 0}`] = new Swiper(`.template-banner-swiper${this.temp_index || 0}`, {
                                            initialSlide: 0,
                                            slidesPerView: 3,
                                            spaceBetween: 10,
                                            autoplay: {
        ...
        mounted () {
            if (this.temp_item.data && this.temp_item.data.show_style == 2) {
                this[`template_banner_swiper${this.temp_index}`] = new Swiper(`.template-banner-swiper${this.temp_index}`, {
                    initialSlide: 0,
                    slidesPerView: 3,
                    spaceBetween: 10,
                    autoplay: {
