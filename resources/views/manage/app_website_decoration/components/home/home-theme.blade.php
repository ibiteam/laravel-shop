<style>
    /*主题广告*/
    .template-theme {  }
    .template-theme ul { padding: 0 15px; }
    .template-theme ul li { margin-bottom: 10px; border-radius: 20px; overflow: hidden; }
</style>

<script type="text/x-template" id="home-theme-template">
    <div class="temp-item" :class="{ 's-flex ai-ct jc-ct': temp_item.activity_info && !temp_item.activity_info.length, active: active_index == temp_item.id, disabled: !temp_item.is_show }">
        ...
        <template>
            ...
            <div class="template-theme public-box" v-else>
                ...
                <ul class="s-flex ai-ct flex-wrap jc-bt">
                    <li class="img-set-full" :style="{ width: temp_item.data.width + 'px', height: temp_item.data.height + 'px' }" v-for="item in temp_item.data.items">
                    ...
                    <div class="drawer-item">
                        ...
                        <div class="drawer-item-dd" style="padding: 0 20px;">
                            ...
                            <el-form-item label="图片宽高" label-width="80px" :prop="'height'" :rules="
                                    [
                                        ...
                                        { validator: (rule, value, callback) => {
                                            if (value < 150 || value > 200) {
                                                callback(new Error('图片高度范围是150px~200px'));
                                            } else if (isNaN(value)) {
                                ...
                                <p class="warning-text" style="line-height: 2; margin: 10px 0;">高度范围：150px~200px，只支持设置为整数</p>
                            </el-form-item>
                        </div>
                    </div>
                    <div class="drawer-item" v-if="templateSetForm.data">
                        <div class="drawer-item-dt">
                            <h1>广告位数据</h1>
                        </div>
                        <div class="drawer-item-dd">
                                ...
                                <dd class="s-flex flex-dir ai-ct">
                                    <ul class="s-flex ai-ct" style="height: 90px; padding: 0 40px;" v-for="(item, index) in templateSetForm.data">
                                        ...
                                        <li class="s-flex ai-ct jc-ct flex-1" style="width: 400px;">
                                            <el-form-item>
                                                <el-select class="radius-right-none__input border-right-none__input" filterable clearable v-model="item.alias" placeholder="请选择" @change="item.value = '', item.url.default_selection_data = [], item.alias == 'yi_qi_xiu' && handleRemoteSearchChange({ path: '{!! route('manage.app_web_decoration.router_search') !!}', method: 'post', params: { keywords: '', alias: item.alias }, parent: `data.${index}.url`, target: 'default_selection_data' }), clearFormValidate(`data_url_value${index}`)" @clear="item.value = ''" style="width: 200px;">
                                                    <el-option v-for="option in options" :key="option.alias" :label="option.name" :value="option.alias"></el-option>
                                                </el-select>
                                            </el-form-item>
...
<script>
     ...
     * initial_index 指定选中项
     * **/
    Vue.component('home-theme-template', {
        props: {
            temp_index: {
                type: Number,
                default: 0
