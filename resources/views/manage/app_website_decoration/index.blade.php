            ...
            <ul class="s-flex ai-ct">
                ...
                <li class="s-flex ai-ct jc-ct">
                    ...
                    <span class="el-dropdown-link">
                        <template>
                           <span v-if="version === '1'">买家版</span>
                           <span v-else-if="version === '2'">卖家版</span>
                           <span v-else>所属板块</span>
                        </template>
                       <i class="el-icon-arrow-down el-icon--right"></i>
                    </span>
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item command="" :class="version===''?'select-item':''">所属板块<i
                                        class="el-icon-check" v-if="version===''"></i></el-dropdown-item>
                            <el-dropdown-item command="1" :class="version==='1'?'select-item':''">买家版<i
                                        class="el-icon-check" v-if="version==='1'"></i></el-dropdown-item>
                            <el-dropdown-item command="2" :class="version==='2'?'select-item':''">卖家版<i
                                        class="el-icon-check" v-if="version==='2'"></i></el-dropdown-item>
                        </el-dropdown-menu>
                    </el-dropdown>
                </li>
                ...
                <li class="s-flex ai-ct jc-ct">url地址</li>
            ...
            <ul class="s-flex ai-ct" v-for="(item,index) in tableData">
                ...
                <li style="text-align: left">
                    <div class="s-flex ai-ct jc-fs">
                        ...
                        <div style="">
                            <p>@{{ item.name }}</p>
                            <p v-if="item.is_dir" style="font-size: 12px; color: #f71111;">共@{{ item.children_count
                                }}个页面</p>
                        </div>
                    </div>
                ...
                <li class="s-flex ai-ct jc-ct">
                    <span v-if="item.version == 1">买家版</span>
                    <span v-else-if="item.version == 2">卖家版</span>
                    <span v-else>--</span>
                </li>
                <li class="s-flex ai-ct jc-ct">移动端</li>
                <li class="s-flex ai-ct jc-ct">@{{ item.type_message }}</li>
                <li class="s-flex ai-ct jc-ct">
                    <div v-if="item.url && item.alias != 'info_class'">
                        <p style="margin-bottom: 10px;color: #337ab7;font-size: 14px;"><a :href="item.url"
                                                                                          target="_blank">预览</a></p>
                        <p @click="copyData(item.url,'复制URL成功！')" class="copyAddress"
                           style="cursor: pointer;color: #409EFF;font-size: 14px;font-weight: 500;"><em
                                    style="font-style: normal;">复制地址</em></p>
                    </div>
                    <p v-else>--</p>
                </li>
                ...
                <li class="s-flex ai-ct jc-fe flex-1">
                    <div class="s-flex flex-wrap jc-ct" v-if="item.is_show_bottom">
                        <el-button type="" v-if="item.is_show_edit" size="small"
                                   @click="openUpdateInfoVisible(item.type,'update',item.id)">页面设置
                        </el-button>
                        <el-button type="primary" v-if="item.is_allow_decoration" size="small"
                                   @click="openDesignVisible(item)">页面装修
                        </el-button>
                        <template v-if="item.is_dir">
                            <el-button type="warning" size="small" @click="openUpdateInfoVisible(item.type,item.alias)">
                                创建页面
                            </el-button>
                            <el-button type="success" size="small" @click="openDirVisible(item)">查看页面</el-button>
                        </template>
                    </div>
                    <div v-else>
                        暂不支持装修
    ...
    <el-dialog
        ...
        <div class="set-main">
            <div class="set-base">
                ...
                <div>
                    <el-form label-width="135px" class="ruleForm">
                        ...
                        <el-form-item label="页面类型" required style="margin-bottom: 20px;">
                            <el-checkbox-group v-model="formSet.type" disabled="true">
                                <el-checkbox v-for="check_item in check_data" v-if="checkIsNotShow(check_item.value)"
                                             :label="check_item.value">@{{
                                    check_item.label }}
                                </el-checkbox>
                            </el-checkbox-group>
                        </el-form-item>
                        <el-form-item label="直播广场数据处理" required style="margin-bottom: 20px;" v-if="formSet.alias == 'live_square'">
                            <el-switch
                                    v-model="formSet.is_merge_live"
                                    active-color="#13ce66"
                            ...
                            ></el-switch>
                            <dl>
                                <dd class="contract_elwarr">影响范围：直播广场，只处理直播中、预告中的数据，PC端和移动端共用控制条件</dd>
                                <dd class="contract_elwarr">【合并】获取多多系所有商城的直播房间数据</dd>
                                <dd class="contract_elwarr">【分离】仅获取当前多多商城的直播房间数据</dd>
                            </dl>
                        </el-form-item>
        ...
        var vm = new Vue({
            ...
            methods: {
                getData(page = 1) {
                    this.form.page = page;
                    this.loading = true;
                    this.doGet('{!! route('manage.app_web_decoration.index') !!}', this.form).then(res => {
                        if (res.code == 200) {
                ...
                openUpdateInfoVisible(type, identification = "update", app_website_decoration_id = null) {
                    if (identification !== "update") { // 数据新增
                        ...
                        this.formSet.type = type
                        this.is_show_form_set_show = true;
                    } else { // 数据编辑
                        this.dialog_title = '页面设置';
                        this.doPost('{!! route('manage.app_web_decoration.edit') !!}', {id: app_website_decoration_id}).then(res => {
                ...
                storeData() {
                    ...
                    this.doPost(store_url, this.formSet).then(res => {
                        ...
                        if (res.code == 200) {
                            ...
                            if (this.formSet.alias == '{{ \App\Models\AppWebsiteDecoration::ALIAS_INDUSTRIAL }}') {
                                parent.openTab('产业链专题合集', '{!! route('manage.app_web_decoration.children_index') !!}' + '?alias=' + this.formSet.alias);
                            } else if (this.formSet.alias == '{{ \App\Models\AppWebsiteDecoration::ALIAS_PUBLIC }}') {
                ...
                handleVersionCommand(command) {
                    this.form.version = this.version = command;
                    this.getData()
                },
                /**
                 * 复制别名 act
                ...
                openDesignVisible(item) {
                    let url = "{!! route('manage.app_web_decoration.decoration') !!}" + "?id=" + item.id;
                    console.log(url)
                ...
                checkIsNotShow(value) {
                    let res = this.formSet.type.findIndex((item, index) => {
                        return item == value
                    })
                    return res >= 0;
                },
            ...
            mounted() {
                ...
                parent.classList.add('decoration-list-parent')
                this.getData();
            },
            destroyed () {
                const parent = document.querySelector('.app-container')
                parent.classList.remove('decoration-list-parent')
            }
        });
    </script>
...
@section('css')
    <style>
        ...
        .decoration-list ul li:last-child button { margin-bottom: 10px; }
