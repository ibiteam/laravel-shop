    ...
    >
        ...
        <el-table-column label="操作">
            <template #default="{row}">
                <el-button link type="primary" @click="openStoreDialog(row)">编辑</el-button>
                <el-button link type="primary" @click="openAdminOperationLog(row.user_name)">操作日志</el-button>
            </template>
        </el-table-column>
    </page-table>
...
const onSubmit = () => {
    submitFormRef.value.validate((valid) => {
        if (valid) {
            ...
            adminUserStore(submitForm).then((res: any) => {
...
const getData = (page: number = defaultPage.page) => {
    ...
    adminUserIndex(params).then((res: any) => {
        loading.value = false;
        if (cns.$successCode(res.code)) {
